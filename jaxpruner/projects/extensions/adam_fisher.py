...
class AdamBasedFisherPruning(jaxpruner.BaseUpdater):
  ...
  def calculate_scores(self, params, sparse_state=None, grads=None):
    ...
    scores = jax.tree_util.tree_map(lambda p, nu: p**2 * nu, params, adamstate.nu)
    del sparse_state
    return scores
  ...
  def update_state(self, sparse_state, params, grads):
    ...
    if self.reset_adam:
      ...
      adamstate = adamstate._replace(
          mu=jax.tree_util.tree_map(jnp.zeros_like, adamstate.mu),
          nu=jax.tree_util.tree_map(jnp.zeros_like, adamstate.nu),
      )
